...
public class AcaciaTreeGen implements MapGen {
	public static final MapGen INSTANCE = new AcaciaTreeGen(Blocks.ACACIA_LOG.getDefaultState(), Blocks.ACACIA_LEAVES.getDefaultState().with(Properties.DISTANCE_1_7, 1), 8);
	private final BlockState log;
	private final BlockState leaves;
	private final int height;

	public AcaciaTreeGen(BlockState log, BlockState leaves, int height) {
		this.log = log;
		this.leaves = leaves;
		this.height = height;
	}
	...
	public void generate(ServerWorldAccess world, BlockPos pos, Random random) {
		...
		for (int y = 0; y <= height; y++) {
			...
			if (y > branchThreshold && random.nextInt(1) == 0) {
				BlockPos local = mutable.toImmutable();
				double theta = random.nextDouble() * Math.PI * 2;
				// TODO: scale with height
				int branchLength = random.nextInt(4) + 3;
